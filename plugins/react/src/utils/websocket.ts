...
export function redactUrl(url: string): string {
  try {
    ...
    return url.replace(
      /[?&](api_key|access_token|guest_token)=[^&]*/g,
      (match) => {
...
/**
 * Used to build the WebSocket URL for a conversation.
 * @param baseUrl
 * @param websocketUrl
 ...
 * @returns
 */
export function createWebSocketConversationUrl(
  baseUrl: string,
  ...
  const tenantParam = tenant
    ? `&x-tenant-id=${encodeURIComponent(tenant)}`
    : "";
  const langParam = language ? `&lang=${language}` : "&lang=en";

  const wsVersion = websocketUrl ? 2 : 1;
  const conversationPath =
    wsVersion === 1
      ? `conversations/ws/${conversationId}`
      : `ws/conversations/${conversationId}`;

  let wsBaseUrl = wsVersion === 1 ? `${baseUrl.replace("http", "ws")}/api` : websocketUrl;
  wsBaseUrl = wsBaseUrl?.replace(/\/\//g, "/");
  return `${wsBaseUrl}/${conversationPath}?${authParam}&${topicsQuery}${tenantParam}${langParam}`?.replace(/\/\//g, "/");
}
