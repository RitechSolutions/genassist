...
const WorkflowSettingsDialog: React.FC<WorkflowSettingsDialogProps> = ({
  ...
  const selected = nodeStyle ?? DEFAULT_NODE_STYLE;
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-[520px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Preferences for how this workflow is displayed. Saved with the
            workflow.
          </DialogDescription>
        </DialogHeader>

        <div className="space-y-3 py-1">
          <div className="text-sm font-medium text-foreground">Node style</div>
          <div
            role="radiogroup"
            ...
            {STYLE_OPTIONS.map((option) => {
              ...
              return (
                <button
                  ...
                  onClick={() => onNodeStyleChange(option.value)}
                  className={cn(
